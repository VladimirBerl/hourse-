...
import { UserRole, NewsItem, User, Poll, PollOption, Quiz, SubscriptionTier } from '../types';
...
const PollComponent: React.FC<{ item: NewsItem, users: User[] }> = ({ item, users }) => {
    ...
    const { submitVote, updateNewsItem } = data;
    const { poll } = item;
    ...
    const handleStopPoll = () => {
        if (!isAdmin || !updateNewsItem || !item.poll) return;
        updateNewsItem(item.id, { poll: { ...item.poll, isStopped: true } });
    };
    ...
    const handleVoteSubmit = (e: React.FormEvent) => {
        ...
        if (selectedOptions.length > 0) {
            submitVote(item.id, 'news', selectedOptions);
        }
    };
...
const NewsPage: React.FC = () => {
    ...
    const [defaultDeletion, setDefaultDeletion] = useState(data?.settings.defaultNewsDeletion || 'none');
    ...
    const { news, addNewsItem, updateNewsItem, deleteNewsItem, settings, updateSettings, locations } = data;
    ...
    const relevantNews = useMemo(() => {
        ...
        if (user.role === UserRole.Admin) {
            ...
            const unpinned = news.filter(n => !n.isPinned);

            const sortAdminItems = (items: NewsItem[]) => {
                const scheduled = items.filter(n => n.publishTimestamp && new Date(n.publishTimestamp) > now);
                const published = items.filter(n => !n.publishTimestamp || new Date(n.publishTimestamp) <= now);
        ...
        const filteredForUser = news.filter(n => {
            ...
            if (n.targetSubscriptionTiers && n.targetSubscriptionTiers.length > 0) {
                if (!user.subscription || !n.targetSubscriptionTiers.includes(user.subscription.tier)) {
                    return false;
                }
            }

            // Role check
            const roleMatch = !n.targetRoles || n.targetRoles.length === 0 || n.targetRoles.includes(user.role);
            if (!roleMatch) return false;

            // Location check
            const hasLocationTarget = (n.targetCountries && n.targetCountries.length > 0) ||
                                      (n.targetRegions && n.targetRegions.length > 0) ||
                                      (n.targetCities && n.targetCities.length > 0);

            if (!hasLocationTarget) {
                return true; // Global post, location matches
            }

            const countryMatch = n.targetCountries && n.targetCountries.includes(user.country);
            const regionMatch = n.targetRegions && n.targetRegions.includes(user.region);
            const cityMatch = n.targetCities && n.targetCities.includes(user.city);

            return countryMatch || regionMatch || cityMatch;
        });

        const pinned = filteredForUser.filter(n => n.isPinned);
        const unpinned = filteredForUser.filter(n => !n.isPinned);

        const sortByDateDesc = (a: NewsItem, b: NewsItem) => new Date(b.publishTimestamp || b.timestamp).getTime() - new Date(a.publishTimestamp || a.timestamp).getTime();
    ...
    const getTargetUsers = (item: NewsItem) => {
    ...
    const handleToggle = (id: string) => {
        setExpandedId(prev => {
            ...
            if (newExpandedId !== null) {
                if (!user.readNewsIds?.includes(id)) {
                    updateUserSilently({ readNewsIds: [...(user.readNewsIds || []), id] });
                }
            }
    ...
    const confirmDelete = async () => {
        if (deletingNewsId) {
            await deleteNewsItem(deletingNewsId);
            setDeletingNewsId(null);
        }
    };
    
    const handleSave = async (data: Omit<NewsItem, 'id' | 'authorId' | 'timestamp'>) => {
        if (editingNews) {
            await updateNewsItem(editingNews.id, data);
        } else {
    ...
    const handleSaveDefaultDeletion = async (e: React.FormEvent) => {
        e.preventDefault();
        await updateSettings({ defaultNewsDeletion: defaultDeletion });
    ...
    return (
        <div className="space-y-6">
            <PublicationEditorModal 
                ...
                allUsers={users}
                locations={locations}
                defaultAutoDeleteOption={settings.defaultNewsDeletion}
            />
            <ConfirmationModal
            ...
            />
             ...
             {user.role === UserRole.Admin && user.permissions?.canManageNews && (
                <div className="flex flex-wrap justify-between items-center gap-4">
                    <form onSubmit={handleSaveDefaultDeletion} className="flex flex-wrap items-center gap-4 bg-gray-50 p-3 rounded-lg">
                        <label htmlFor="default-deletion-news" className="text-sm font-medium text-gray-700 whitespace-nowrap">Удаление постов по умолчанию:</label>
                        <select id="default-deletion-news" value={defaultDeletion} onChange={e => setDefaultDeletion(e.target.value)} className="w-full max-w-xs px-2 py-1 border border-gray-300 rounded-md bg-white">
            ...
            <div className="space-y-4">
                {relevantNews.length > 0 ? relevantNews.map(newsItem => {
                    const isRead = user.readNewsIds?.includes(newsItem.id) ?? true;
                    const targetUsers = getTargetUsers(newsItem);
                    const sentToCount = targetUsers.length;
                    const readCount = targetUsers.filter(u => u.readNewsIds?.includes(newsItem.id)).length;
                    const isScheduled = newsItem.publishTimestamp && new Date(newsItem.publishTimestamp) > new Date();

                    const imagesAboveTitle = newsItem.images?.filter(img => img.position === 'aboveTitle') || [];
                    const imagesBeforeContent = newsItem.images?.filter(img => img.position === 'beforeContent') || [];
                    const imagesAfterContent = newsItem.images?.filter(img => img.position === 'afterContent') || [];

                    return (
                         <div key={newsItem.id} className={`bg-white rounded-lg shadow-md overflow-hidden border-2 ${newsItem.isPinned ? 'border-brand-primary' : 'border-transparent'}`}>
                             {imagesAboveTitle.map(img => <img key={img.id} src={img.url} alt={newsItem.title} className="w-full h-auto object-cover" />)}
                            <div className="p-4 cursor-pointer" onClick={() => handleToggle(newsItem.id)}>
                               <div className="flex justify-between items-start">
                                    <div className="flex-grow pr-4">
                                        <div className="flex items-center flex-wrap gap-2">
                                            {newsItem.isPinned && <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-brand-primary mr-2 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M16 9V4h1c.55 0 1-.45 1-1s-.45-1-1-1H7c-.55 0-1 .45-1 1s.45 1 1 1h1v5c0 1.66-1.34 3-3 3v2h5.97v7l1 1 1-1v-7H19v-2c-1.66 0-3-1.34-3-3z"/></svg>}
                                            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                                <span>{newsItem.title}</span>
                                                {newsItem.targetSubscriptionTiers?.includes(SubscriptionTier.Maximum) && <AnimatedStar />}
                                            </h3>
                                        ...
                                        {user.role === UserRole.Admin ? (
                                            <div>
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {new Date(newsItem.timestamp).toLocaleString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                    ...
                                                    {isScheduled && <span> / Публикация: {new Date(newsItem.publishTimestamp!).toLocaleString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>}
                                                </p>
                                                 {newsItem.deletionTimestamp && (
                                                    <p className="text-xs text-red-500 mt-1">
                                                        Удаление поста: {new Date(newsItem.deletionTimestamp).toLocaleString('ru-RU')}
                                                    </p>
                                                )}
                                            </div>
                                        ) : (
                                             <p className="text-xs text-gray-500 mt-1">
                                                {new Date(newsItem.publishTimestamp || newsItem.timestamp).toLocaleString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                            </p>
                                        )}
                                    </div>
                                     <div className="flex items-center flex-wrap gap-x-2 sm:gap-x-4 gap-y-1 flex-shrink-0 justify-end">
                                        {user.role === UserRole.Admin && user.permissions?.canManageNews && (
                                             <>
                                                <button onClick={(e) => { e.stopPropagation(); updateNewsItem(newsItem.id, { isPinned: !newsItem.isPinned }); }} title={newsItem.isPinned ? "Открепить" : "Закрепить"} className={`p-1 rounded-full hover:bg-gray-100 ${newsItem.isPinned ? 'text-brand-primary' : 'text-gray-400'}`}>
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); handleEdit(newsItem); }} title="Редактировать" className="font-semibold text-blue-600 hover:text-blue-800">Ред.</button>
                                                <button onClick={(e) => { e.stopPropagation(); handleDelete(newsItem.id); }} title="Удалить" className="font-semibold text-red-600 hover:text-red-800">Удал.</button>
                                            </>
                                        )}
                                        {!isRead && user.role !== UserRole.Admin && <span className="w-3 h-3 bg-red-500 rounded-full flex-shrink-0" title="Новое"></span>}
                                        <button className="focus:outline-none"><svg className={`w-5 h-5 text-gray-500 transform transition-transform ${expandedId === newsItem.id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>
                                    </div>
                               </div>
                            </div>
                            <div className={`transition-all duration-700 ease-in-out overflow-hidden ${expandedId === newsItem.id ? 'max-h-[2000px]' : 'max-h-0'}`}>
                                <div className="px-4 pb-4">
                                     {imagesBeforeContent.map(img => <img key={img.id} src={img.url} alt={newsItem.title} className="w-full h-auto object-cover rounded-lg my-4" />)}
                                    <div className="border-t pt-4 text-gray-700 whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: linkify(newsItem.content) }} />
                                    {imagesAfterContent.map(img => <img key={img.id} src={img.url} alt={newsItem.title} className="w-full h-auto object-cover rounded-lg mt-4" />)}
                                    {newsItem.poll && <PollComponent item={newsItem} users={users} />}
                                    {newsItem.quiz && <QuizComponent item={newsItem} itemType="news" />}
                                </div>
                            </div>
                        </div>
                    );
