...
const EditLocationNameModal: React.FC<EditLocationNameModalProps> = ({
  isOpen,
  onClose,
  onSave,
  itemType,
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSubmit}>
          <div className="p-6 border-b">
            <h2 className="text-xl font-bold text-gray-800">Редактировать {itemType}</h2>
          </div>
...
const LocationManager: React.FC<LocationManagerProps> = ({ users, updateUsers }) => {
  ...
  const [editModal, setEditModal] = useState<{
    ...
    value: string;
    context?: { country?: string; region?: string };
  }>({ isOpen: false, type: 'country', value: '' });
  ...
  const regions =
    selectedCountry && locations[selectedCountry]
      ? Object.keys(locations[selectedCountry])
          .filter((r) => r.toLowerCase().includes(regionSearch.toLowerCase()))
          .sort((a, b) => a.localeCompare(b, 'ru'))
      : [];
  const cities =
    selectedCountry && selectedRegion && locations[selectedCountry]?.[selectedRegion]
  ...
  const handleAddItem = async (type: 'country' | 'region' | 'city', name: string) => {
    ...
    if (type === 'country') {
      if (!newLocations[trimmedName]) {
        ...
        alert('Такая страна уже существует.');
        return;
      }
      ...
      if (!newLocations[selectedCountry][trimmedName]) {
        ...
        alert('Такой регион уже существует в этой стране.');
        return;
      }
      ...
      if (!cityList.includes(trimmedName)) {
        ...
        alert('Такой город уже существует в этом регионе.');
        return;
      }
    }
  ...
  const handleSaveEdit = async (newValue: string) => {
    ...
    if (type === 'country') {
      ...
      if (newLocations[context.country][newValue]) {
    ...
    await Promise.all([updateLocations(newLocations), updateUsers(updatedUsers)]);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <EditLocationNameModal
        ...
        onClose={() => setEditModal({ isOpen: false, type: 'country', value: '' })}
        onSave={handleSaveEdit}
        itemType={editModal.type === 'country' ? 'страну' : editModal.type === 'region' ? 'регион' : 'город'}
        initialValue={editModal.value}
      />
      <ConfirmationModal
        isOpen={deleteModal.isOpen}
        onClose={() => setDeleteModal({ isOpen: false, type: 'country', value: '' })}
        onConfirm={handleConfirmDelete}
        title={`Удалить ${
          deleteModal.type === 'country' ? 'страну' : deleteModal.type === 'region' ? 'регион' : 'город'
        }`}
        message={`Вы уверены? Удаление ${
          deleteModal.type === 'country'
            ? 'страны приведет к удалению всех ее регионов и городов'
            : deleteModal.type === 'region'
            ? 'региона приведет к удалению всех его городов'
            : ''
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col space-y-3 p-3 bg-gray-50 rounded-lg border">
          ...
          <div className="relative flex-shrink-0">
            <input
              ...
              className="w-full px-3 py-1.5 pl-8 border border-gray-300 rounded-md shadow-sm text-sm"
            />
            <svg
              className="h-4 w-4 text-gray-400 absolute top-1/2 left-2.5 -translate-y-1/2"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
            ...
            {countries.map((c) => (
              <div
                ...
                className={`flex items-center justify-between p-2 rounded-md cursor-pointer ${
                  selectedCountry === c ? 'bg-brand-secondary text-white' : 'bg-white hover:bg-gray-100'
                }`}
              >
                <span className="flex-grow">{c}</span>
                <ActionButtons
                  onEdit={() => setEditModal({ isOpen: true, type: 'country', value: c })}
                  onDelete={() => setDeleteModal({ isOpen: true, type: 'country', value: c })}
                />
              </div>
            ))}
          </div>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleAddItem('country', newCountryName);
            }}
            className="flex-shrink-0 flex space-x-2 pt-2 border-t"
          >
            <input
              type="text"
              ...
              className="flex-grow w-full px-3 py-1.5 border border-gray-300 rounded-md shadow-sm text-sm"
            />
            <button
              type="submit"
              className="bg-brand-accent text-white px-3 py-1.5 rounded-md text-sm font-semibold hover:bg-green-600"
            >
              +
            </button>
          </form>
        ...
        <div
          className={`flex flex-col space-y-3 p-3 rounded-lg border ${
            !selectedCountry ? 'bg-gray-100' : 'bg-gray-50'
          }`}
        >
          ...
          <div className="relative flex-shrink-0">
            <input
              ...
              className="w-full px-3 py-1.5 pl-8 border border-gray-300 rounded-md shadow-sm text-sm disabled:bg-gray-200"
            />
            <svg
              className="h-4 w-4 text-gray-400 absolute top-1/2 left-2.5 -translate-y-1/2"
              xmlns="http://www.w3.org/2000/svg"
            ...
            {selectedCountry ? (
              regions.map((r) => (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded-md cursor-pointer ${
                    selectedRegion === r ? 'bg-brand-secondary text-white' : 'bg-white hover:bg-gray-100'
                  }`}
                >
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleAddItem('region', newRegionName);
            }}
            className="flex-shrink-0 flex space-x-2 pt-2 border-t"
          >
            <input
              ...
              disabled={!selectedCountry}
              className="flex-grow w-full px-3 py-1.5 border border-gray-300 rounded-md shadow-sm text-sm disabled:bg-gray-200"
            />
            <button
              type="submit"
              disabled={!selectedCountry}
              className="bg-brand-accent text-white px-3 py-1.5 rounded-md text-sm font-semibold hover:bg-green-600 disabled:bg-gray-400"
            >
              +
            </button>
          </form>
        ...
        <div
          className={`flex flex-col space-y-3 p-3 rounded-lg border ${
            !selectedRegion ? 'bg-gray-100' : 'bg-gray-50'
          }`}
        >
                  ...
                  <ActionButtons
                    onEdit={() =>
                      setEditModal({
                        ...
                        type: 'city',
                        value: city,
                        context: { country: selectedCountry, region: selectedRegion },
                      })
                    }
                    onDelete={() =>
                      setDeleteModal({
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleAddItem('city', newCityName);
            }}
            className="flex-shrink-0 flex space-x-2 pt-2 border-t"
          >
            <input
...
const AddAdminModal: React.FC<{
  ...
  onClose: () => void;
  onAdd: (
    details: Omit<User, 'id' | 'registrationDate' | 'linkedUsers' | 'role' | 'passwordHash' | 'salt'> & {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSubmit}>
          <div className="p-6 border-b">
            <h2 className="text-xl font-bold text-gray-800">Добавить администратора</h2>
          </div>
...
const EditAdminNotificationsModal: React.FC<{
  ...
  const adminSettings: {
    ...
    { type: 'adminMessage', label: 'Сообщения в техподдержку', requiredPermission: 'canViewMessages' },
    {
      ...
      label: 'Объявления на модерацию',
      requiredPermission: 'canManageAnnouncements',
    },
    {
      type: 'passwordResetRequest',
      label: 'Запросы на сброс пароля',
      requiredPermission: 'canResetUserPasswords',
    },
    {
      type: 'newLocationRequest',
      label: 'Запросы на проверку локаций',
      requiredPermission: 'canManageUsers',
    },
  ];

  const [currentSettings, setCurrentSettings] = useState<
    Partial<Record<NotificationType, NotificationSetting>>
  >({});
  ...
  const availableSettings = adminSettings.filter(
    (s) => !s.requiredPermission || admin.permissions?.[s.requiredPermission]
  );
          ...
          <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
            {availableSettings.length > 0 ? (
              ...
              <p className="text-sm text-center text-gray-500 py-4">
                У этого администратора нет доступов, требующих уведомлений.
              </p>
            )}
          </div>
          <div className="p-6 border-t flex justify-end space-x-3">
...
const NotificationSettings: React.FC = () => {
  ...
  if (!auth || !auth.user || !auth.updateUser) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-md">
  ...
  const handleToggle = (type: NotificationType, isChecked: boolean) => {
    const newSettings = { ...safeNotificationSettings };
    if (isChecked) {
  ...
  const settingsToShow =
    user.role === UserRole.Admin
      ? adminSettings.filter((s) => !s.requiredPermission || safePermissions[s.requiredPermission])
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {typeof Notification !== 'undefined' && permissionStatus !== 'granted' && (
        <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 mb-6">
          ...
          {permissionStatus === 'default' && (
            <button
              ...
              className="mt-2 bg-brand-primary text-white px-3 py-1.5 rounded-md text-sm font-semibold hover:bg-brand-secondary transition"
            >
        ...
        {settingsToShow.map((s) => (
          <div key={s.type} className="py-3 flex items-center justify-between">
            ...
            <label className="flex items-center space-x-3 cursor-pointer">
              <input
                type="checkbox"
                checked={!!safeNotificationSettings[s.type]}
                onChange={(e) => handleToggle(s.type, e.target.checked)}
                className="h-5 w-5 text-brand-primary border-gray-300 rounded focus:ring-brand-primary"
              />
            </label>
...
const WelcomePageEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (data?.welcomePageContent) {
      setFormData(data.welcomePageContent);
    }
  }, [data?.welcomePageContent]);

  if (!data || !data.updateWelcomePageContent) {
  ...
  const handleListChange = (
    ...
    listName: 'forTrainersList' | 'forStudentsList'
  ) => {
  ...
  const handleTestimonialChange = (id: string, field: 'quote' | 'name' | 'role', value: string) => {
    const newTestimonials = formData.testimonials.map((t) => (t.id === id ? { ...t, [field]: value } : t));
  ...
  const removeTestimonial = (id: string) => {
    setFormData((prev) =>
      prev ? { ...prev, testimonials: prev.testimonials.filter((t) => t.id !== id) } : null
    );
  };
  ...
  const ImageInput: React.FC<{ name: keyof WelcomePageContent; label: string; tooltipText: string }> = ({
    name,
    label,
    tooltipText,
  }) => (
    <div>
      ...
      <div className="flex items-center space-x-2 mt-1">
        ...
        <button
          ...
          className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 text-sm whitespace-nowrap"
        >
          Загрузить
        </button>
      </div>
    ...
    <form onSubmit={handleSave} className="space-y-8">
      <fieldset className="space-y-4 p-4 border rounded-lg">
        ...
        <ImageInput
          ...
          tooltipText="Рекомендуемый размер: 1920x1080px. Формат: JPG, PNG. Изображение будет затемнено для читаемости текста."
        />
      </fieldset>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3 p-3 border rounded-md">
            ...
            <div>
              ...
              <input
                type="text"
                name="forTrainersTitle"
                value={formData.forTrainersTitle}
                onChange={handleChange}
                className="w-full mt-1 p-2 border rounded"
              />
            </div>
            <div>
              <label className="text-xs font-medium">Подзаголовок</label>
              <textarea
                name="forTrainersSubtitle"
                value={formData.forTrainersSubtitle}
                onChange={handleChange}
                rows={2}
                className="w-full mt-1 p-2 border rounded"
              />
            ...
            <div>
              <label className="text-xs font-medium">Список (каждый с новой строки)</label>
              <textarea
                value={(formData.forTrainersList ?? []).join('\n')}
                onChange={(e) => handleListChange(e, 'forTrainersList')}
                rows={4}
                className="w-full mt-1 p-2 border rounded"
              />
            </div>
            <ImageInput
              name="forTrainersImageUrl"
              label="Изображение"
              tooltipText="Рекомендуемый размер: 800x600px (соотношение 4:3). Формат: JPG, PNG."
            />
          </div>
          <div className="space-y-3 p-3 border rounded-md">
            ...
            <div>
              ...
              <input
                type="text"
                name="forStudentsTitle"
                value={formData.forStudentsTitle}
                onChange={handleChange}
                className="w-full mt-1 p-2 border rounded"
              />
            </div>
            <div>
              <label className="text-xs font-medium">Подзаголовок</label>
              <textarea
                name="forStudentsSubtitle"
                value={formData.forStudentsSubtitle}
                onChange={handleChange}
                rows={2}
                className="w-full mt-1 p-2 border rounded"
              />
            ...
            <div>
              <label className="text-xs font-medium">Список (каждый с новой строки)</label>
              <textarea
                value={(formData.forStudentsList ?? []).join('\n')}
                onChange={(e) => handleListChange(e, 'forStudentsList')}
                rows={4}
                className="w-full mt-1 p-2 border rounded"
              />
            </div>
            <ImageInput
              name="forStudentsImageUrl"
              label="Изображение"
              tooltipText="Рекомендуемый размер: 800x600px (соотношение 4:3). Формат: JPG, PNG."
            />
          </div>
      ...
      <fieldset className="space-y-4 p-4 border rounded-lg">
        ...
        <div>
          ...
          <div className="flex items-center space-x-2 mt-1">
            <input
              type="text"
              name="forWhomSectionBg"
              value={formData.forWhomSectionBg || ''}
              onChange={handleChange}
              placeholder="Например, #f9fafb или url(...)"
              className="w-full p-2 border rounded"
            />
            <button
              type="button"
              onClick={() => handleFileUpload('forWhomSectionBg')}
              className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 text-sm whitespace-nowrap"
            >
              Загрузить
            </button>
        ...
        <div>
          ...
          <div className="flex items-center space-x-2 mt-1">
            <input
              type="text"
              name="testimonialsSectionBg"
              value={formData.testimonialsSectionBg || ''}
              onChange={handleChange}
              placeholder="Например, #2c5282 или url(...)"
              className="w-full p-2 border rounded"
            />
            <button
              type="button"
              onClick={() => handleFileUpload('testimonialsSectionBg')}
              className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 text-sm whitespace-nowrap"
            >
              Загрузить
            </button>
      ...
      <fieldset className="space-y-4 p-4 border rounded-lg">
        ...
        <button type="button" onClick={addTestimonial} className="text-sm text-brand-primary hover:underline">
          + Добавить отзыв
        </button>
      </fieldset>
      ...
      <div className="flex justify-end items-center space-x-4 sticky bottom-0 bg-white py-4 px-6 -mx-6 -mb-6 border-t rounded-b-lg">
        ...
        <button
          type="submit"
          className="bg-brand-primary text-white font-semibold px-6 py-2 rounded-lg shadow hover:bg-brand-secondary transition-colors"
        >
          Сохранить изменения
        </button>
      </div>
...
const AdminSettings: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = usePersistentColumns(
    'adminSettingsColumns',
    adminColumnsConfig
  );
  ...
  // Check if DataContext is needed for the current tab
  const needsDataContext = activeTab === 'locations' || activeTab === 'welcome' || activeTab === 'legal';
  if (needsDataContext && !data) {
    return <div className="bg-white p-6 rounded-lg shadow-md">Загрузка данных...</div>;
  }
  ...
  const handleSavePermissions = async (
    userId: string,
    permissions: Omit<AdminPermissions, 'canManageAdmins'>
  ) => {
    ...
    await silentlyUpdateUserInList(userId, {
  ...
  const otherAdmins = users.filter(
    (u) => u.role === UserRole.Admin && u.id !== currentUser.id && !u.isDeleted && u.id !== BACKUP_ADMIN_ID
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mt-6">
        ...
        {activeTab === 'admins' && currentUser.permissions?.canManageAdmins && (
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <AdminSetPasswordModal
              isOpen={!!adminToReset}
              onClose={() => setAdminToReset(null)}
              user={adminToReset}
              onSave={adminResetPassword!}
              title="Сменить пароль для админа"
            />
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
              ...
              <div className="flex items-center space-x-4">
                <ColumnToggler
                  columns={adminColumnsConfig}
                  visibleColumns={visibleColumns}
                  setVisibleColumns={setVisibleColumns}
                />
                <button
                  onClick={() => setAddModalOpen(true)}
                  className="bg-brand-primary text-white font-semibold px-4 py-2 rounded-lg shadow hover:bg-brand-secondary transition-colors flex items-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {otherAdmins.map((admin) => {
                    ...
                    const canResetPassword =
                      (currentUser.id === BACKUP_ADMIN_ID && isMainAdmin) || !isSuperAdmin;
                        ...
                        {visibleColumns.includes('registrationDate') && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">
                              {new Date(admin.registrationDate).toLocaleDateString('ru-RU')}
                            </div>
                          </td>
                        )}
                        {visibleColumns.includes('status') && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                admin.isOnline ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                              }`}
                            >
                              {admin.isOnline ? 'В сети' : 'Не в сети'}
                            </span>
                          </td>
                        )}
                        {visibleColumns.includes('lastLogin') && (
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {admin.lastLogin ? new Date(admin.lastLogin).toLocaleString('ru-RU') : 'N/A'}
                          </td>
                        )}
                        <td className="px-6 py-4 text-sm font-medium">
                          <div className="flex flex-wrap gap-x-4 gap-y-2">
                            ...
                            {canResetPassword ? (
                              ...
                              <span className="text-gray-400 cursor-not-allowed" title="Недоступно">
                                Пароль
                              </span>
                            )}
                            <button
                              onClick={() => handleEditNotificationsClick(admin)}
        ...
        {activeTab === 'locations' && currentUser.permissions?.canManageAdmins && (
          <LocationManager users={users} updateUsers={updateUsers!} />
        )}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <form onSubmit={handlePasswordChange} className="max-w-md space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700" htmlFor="new-password-user">
              Новый пароль
            </label>
            <input
              type="password"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700" htmlFor="confirm-password-user">
          ...
          {message && (
            <p className={`text-sm ${message.includes('успешно') ? 'text-green-600' : 'text-red-600'}`}>
              {message}
            </p>
          )}
          <button
      ...
      <div className="bg-red-50 p-6 rounded-lg shadow-md border border-red-200">
        ...
        <div className="space-y-3 text-sm text-red-700">
          <p>
            <strong>Внимание:</strong> Это действие необратимо. При удалении аккаунта все ваши данные, включая
            историю тренировок, чаты и статистику, будут безвозвратно удалены.
          </p>
          <p>
            Также обращаем ваше внимание, что аккаунты, неактивные более 6 месяцев (без единого входа в
            приложение), подлежат автоматическому удалению.
          </p>
        </div>
        <div className="mt-6">
          <button
